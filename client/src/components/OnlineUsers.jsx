...
const OnlineUsers = () => {
  ...
  // Close menu when user is selected on mobile
  useEffect(() => {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth > 768) {
        setIsMenuOpen(true); // Always show on desktop
      } else {
        setIsMenuOpen(false); // Hide on mobile by default
      }
    };
  ...
  return (
    <>
      ...
      <div className={`online-users-container ${isMenuOpen ? 'open' : ''}`}>
        <div className="online-users-header">
          <h3>Online Users</h3>
          <div className="online-stats">
            <div className="online-count">
