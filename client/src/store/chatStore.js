...
//       Object.keys(privateMessages).forEach((partner) => {
//         const messages = privateMessages[partner];
...
//       Object.keys(updatedMessages).forEach((partner) => {
//         updatedMessages[partner] = updatedMessages[partner].map((msg) =>
//           msg.id === messageId
//             ? {
//                 ...msg,
//                 deleted: true,
...
export const useChatStore = create((set, get) => {
  ...
  return {
    ...
    isSocketInitialized: false,
    imageReconstructionProgress: {},
    replyingTo: null, // Add this to track which message we're replying to
    ...
    setSelectedUser: (user) => {
      saveToStorage(STORAGE_KEYS.SELECTED_USER, user);
      set({ selectedUser: user, replyingTo: null }); // Clear reply when switching users
    },
    ...
    // ========================
    // UPDATED: sendMessage function with reply support
    // ========================
    sendMessage: (message, type = 'text', fileData = null, replyTo = null) => {
      const { username, selectedUser, privateMessages, replyingTo } = get();

      if ((!message && type === 'text') || !selectedUser) return;
      if (type === 'image' && !fileData) return;

      const messageId = generateMessageId();
      const replyMessage = replyTo || replyingTo; // Use provided replyTo or current replyingTo
    ...
    initSocketListeners: () => {
      ...
      // RECEIVE MESSAGE - UPDATED WITH REPLY HANDLING
      socket.on('receivePrivateMessage', (data) => {
        const { privateMessages, username } = get();
        ...
        // Handle different message types (image chunks, etc.)
        if (data.type === 'image-chunk') {
          // ... (keep your existing image chunk handling code)
          return;
        }

        if (data.type === 'image-metadata') {
          // ... (keep your existing image metadata handling code)
          return;
        }
        ...
        if (!messageExists) {
          ...
          console.log(
            `✅ New ${data.type || 'text'} message from ${partner} added${data.replyTo ? ' (reply)' : ''}`,
          );
        }
